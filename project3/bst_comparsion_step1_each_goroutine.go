...
func ComputeHashGoroutine(tree_array *[]Tree, idx int, wg *sync.WaitGroup) {
	defer wg.Done()
	hash := ComputeHash(&((*tree_array)[idx]))
	_ = hash
}
...
func ComputeHashGroup(tree_array *[]Tree, worker int) map[int][]int {
	...
	for i := 0; i < len(*tree_array); i++ {
		wg.Add(1)
		go ComputeHashGoroutine(tree_array, i, &wg)
	}
	// 	if _, ok := ret[hash]; ok {
...
func main() {
	...
	TICK()
	ComputeHashGroup(&tree_array, argu.hash_worker)
	hash_time := TOCK()
